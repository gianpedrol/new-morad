                                    ...
                                    <table class="table table-striped" id="dataTable">
                                        ...
                                        <tbody>
                                            @foreach ($sliders as $index => $slider)
                                                <tr>
                                                    <td>
                                                        <img src="{{ asset($slider->image) }}" width="150px"
                                                            alt="">
                                                    </td>
                                                    <td>{{ $slider->serial }}</td>
                                                    <td>
                                                        @forelse($languages as $key => $language)
                                                            <i
                                                                class="fa {{ $slider->translation($language->code)?->first()?->title ? 'fa-check' : 'fa-edit' }}"></i>
                                                            <a
                                                                href="{{ route('admin.slider.translation', ['id' => $slider->id, 'code' => $language->code]) }}">{{ strtoupper($language->code) }}</a>
                                                            @if (!$loop->last)
                                                                ||
                                                            @endif
                                                        @empty
                                                            <a
                                                                href="{{ route('admin.slider.translation', ['id' => $slider->id, 'code' => $language->code]) }}">{{ strtoupper(config('app.locale')) }}</a>
                                                        @endforelse
                                                    </td>
                                                    <td>
                                                        @if ($slider->status == 1)
                                                            <a href="javascript:;"
                                                                onclick="changeProductCategoryStatus({{ $slider->id }})">
